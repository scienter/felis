...
void main(int argc, char *argv[])
{
   ...
   double minX,minY,dx,dy,x,y,coef,coef2;
   double *U,*Ez;
   ...
   for(step=initial; step<=final; step+=timeStep)
   {
	...
	subCnt[i]=subP;
	 ...
	 start[n]=sum;
      ...
      for(h=0; h<harmony; h++) {
	sprintf(cenFile,"%dField%d",h,step);	
	...
	sprintf(dataName,"U%d",h);	
	  ...
	  for(i=0; i<subCnt[n]; i++) {
	    ...
	    real=U[i*N+cenJ*nx*2+cenI*2+0]*coef;
	    imag=U[i*N+cenJ*nx*2+cenI*2+1]*coef;
	    fprintf(cenOut,"%.15g %g %g\n",z,real,imag);
	  }
